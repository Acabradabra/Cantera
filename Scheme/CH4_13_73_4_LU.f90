!
!       9-step reduced mechanism for lean atmospheric CH4/air
!       QSS species: CH2, CH2(S), HCO, CH2OH
!       Reduced by: Tianfeng Lu
!                  Email: tlu@princeton.edu
!                  Princeton University
!       May 27, 2005
!
! Ref: T. Lu, C. K. Law, Acriterion based on computational singular perturbation for the
...
! Notes (PhD Franzelli 2011):
! The LU mechanism corresponds to the reduced chemical scheme used by Sankaran
! et al. [PCI 2007] in the DNS of the Bunsen flame and is based on the work of Lu and
! Law [CF 2008]. It takes into account 13 resolved species (CH4, O2, CO2, CO, H2O, N2, H2,
...
! relation graph method, the sensitivity analysis and the computational singular perturbation
! approach [Lu & Law, CF 2008]. QSS is assumed for various species in order to obtain algebraic
...
! 1000 K to 1800 K [Sankaran et al., PCI 2007]. Simplified transport properties are assumed.

!>@todo gestion de la precision machine: mettre des _pr partout

SUBROUTINE CH4_13_73_4_LU ( nnode,neqs,rhoinv,tempe,w_spec,wmol,source_spec )
...
! IN/OUT
  INTEGER :: neqs,nnode
  REAL(pr) :: rhoinv(1:nnode)
  REAL(pr) :: tempe(1:nnode)
  REAL(pr) :: w_spec(1:neqs,1:nnode)
  REAL(pr) :: wmol(1:neqs)
  REAL(pr) :: source_spec(1:neqs,1:nnode)
...
! LOCAL
  ...
  REAL(pr) :: CTB(1:73)
  REAL(pr) :: RKLOW(1:5)
  REAL(pr) :: RU, SI_cgs,TMP,TN1,TN2,TN3
  REAL(pr) :: SMALL_H,T,TI,TI2,TN4,TN5,CTOT
  REAL(pr) :: PATM,ALOGT,PFAC,PFAC2,PFAC3
  REAL(pr) :: PRB,PCOR,PRLOG,FCENT,FCLOG,XN
  REAL(pr) :: CPRLOG,FLOG,FC,ABV1,DEN1,ABV2
  REAL(pr) :: DEN2,ABV3,DEN3,ABV4,DEN4,F1
  REAL(pr) :: A12,F2,A21,A24,F3,A31,A13
  REAL(pr) :: F4,A42,TEMP,FF1,AA12,FF2,AA21
  REAL(pr) :: cgs_SI,FAKW,FAKWR,rhoi
  INTEGER :: ispec,n,k,i,j
...
!
    ...
    rhoi = rhoinv(j)
    DO ispec=1,neqs
      C(ispec) = MAX( w_spec(ispec,j), zero )
      C(ispec) = MIN( C(ispec), 1.0_pr/rhoi )
      C(ispec) = C(ispec) / wmol(ispec)
      C(ispec) = C(ispec)* 1.0e-6_pr
!     C(ispec) = max(C(ispec),1.0e-15_pr)
    END DO
...
!
    ...
    END DO
!
    CTB(1) = CTOT + 1.4D0*C(1) + 1.44D1*C(6) + C(9) + 7.5D-1*C(10) + 2.6D0*C(11)
    CTB(2) = CTOT + C(1) + 5.D0*C(6) + C(9) + 5.D-1*C(10) + C(11)
    CTB(9) = CTOT + C(1) + 5.D0*C(4) + 5.D0*C(6) + C(9) + 5.D-1*C(10) + 2.5D0*C(11)
    CTB(16) = CTOT - C(4) - C(6) - 2.5D-1*C(10) + 5.D-1*C(11) - C(13)
    CTB(21) = CTOT - C(1) - C(6) + C(9) - C(11)
    CTB(25) = CTOT - 2.7D-1*C(1) + 2.65D0*C(6) + C(9)
    CTB(29) = CTOT + C(1) + 5.D0*C(6) + C(9) + 5.D-1*C(10) + C(11)
    CTB(30) = CTB(29)
    CTB(32) = CTB(29)
    CTB(34) = CTB(29)
    CTB(39) = CTB(29)
    CTB(71) = CTOT + C(1) - C(6) + C(9) + 5.D-1*C(10) + C(11)


!
!   Fall-off reactions
!
    PRB = RKLOW(1) * CTB(29) / RF(29)
    PCOR = PRB / (1.0 + PRB)
    PRLOG = LOG10(MAX(PRB,SMALL_H))
    FCENT = 3.2D-1*EXP(-T/7.8D1) + 6.8D-1*EXP(-T/1.995D3) + EXP(-5.59D3/T)
    FCLOG = LOG10(MAX(FCENT,SMALL_H))
    XN    = 0.75 - 1.27*FCLOG
    ...
    PRB = RKLOW(2) * CTB(30) / RF(30)
    PCOR = PRB / (1.0 + PRB)
    PRLOG = LOG10(MAX(PRB,SMALL_H))
    FCENT = 2.17D-1*EXP(-T/7.4D1) + 7.83D-1*EXP(-T/2.941D3) + EXP(-6.964D3/T)
    FCLOG = LOG10(MAX(FCENT,SMALL_H))
    XN    = 0.75 - 1.27*FCLOG
    CPRLOG= PRLOG - (0.4 + 0.67*FCLOG)
    FLOG = FCLOG/(1.0 + (CPRLOG/(XN-0.14*CPRLOG))**2)
    ...
    PRB = RKLOW(3) * CTB(32) / RF(32)
    PCOR = PRB / (1.0 + PRB)
    PRLOG = LOG10(MAX(PRB,SMALL_H))
    FCENT = 2.176D-1*EXP(-T/2.71D2) + 7.824D-1*EXP(-T/2.755D3) + EXP(-6.57D3/T)
    FCLOG = LOG10(MAX(FCENT,SMALL_H))
    XN    = 0.75 - 1.27*FCLOG
    CPRLOG= PRLOG - (0.4 + 0.67*FCLOG)
    FLOG = FCLOG/(1.0 + (CPRLOG/(XN-0.14*CPRLOG))**2)
    ...
    PRB = RKLOW(4) * CTB(34) / RF(34)
    PCOR = PRB / (1.0 + PRB)
    PRLOG = LOG10(MAX(PRB,SMALL_H))
    FCENT = 2.813D-1*EXP(-T/1.03D2) + 7.187D-1*EXP(-T/1.291D3) + EXP(-4.16D3/T)
    FCLOG = LOG10(MAX(FCENT,SMALL_H))
    XN    = 0.75 - 1.27*FCLOG
    CPRLOG= PRLOG - (0.4 + 0.67*FCLOG)
    FLOG = FCLOG/(1.0 + (CPRLOG/(XN-0.14*CPRLOG))**2)
    ...
    PRB = RKLOW(5) * CTB(39) / RF(39)
    PCOR = PRB / (1.0 + PRB)
    PRLOG = LOG10(MAX(PRB,SMALL_H))
    FCENT = 6.8D-2*EXP(-T/1.97D2) + 9.32D-1*EXP(-T/1.54D3) + EXP(-1.03D4/T)
    FCLOG = LOG10(MAX(FCENT,SMALL_H))
    XN    = 0.75 - 1.27*FCLOG
    CPRLOG= PRLOG - (0.4 + 0.67*FCLOG)
    FLOG = FCLOG/(1.0 + (CPRLOG/(XN-0.14*CPRLOG))**2)
...
!   CH2
    ABV1 = RB(29) + RB(43) + RF(45) + RB(52) + RB(56) + RB(57)
...
!   CH2OH
    ABV4 = RB(13) + RF(34) + RB(36) + RB(37) + RB(51) + RB(73)
    DEN4 =  RF(13) +  RB(34) +  RF(36) +  RF(37) +  RF(38) +  RF(51) +  RF(73)
    ...
    XQ(1) = (FF1 + AA12*FF2)/(1.0_pr - AA12*AA21)
    XQ(2) = FF2 + AA21*XQ(1)
    XQ(3) = F3 + A31*XQ(1)
    XQ(4) = F4 + A42*XQ(2)

    XQ(1) = MAX(XQ(1),0.0_pr)
    XQ(2) = MAX(XQ(2),0.0_pr)
    XQ(3) = MAX(XQ(3),0.0_pr)
    XQ(4) = MAX(XQ(4),0.0_pr)
...
!
    ...
    source_spec(1,j) = -ROP(3) +ROP(6) +ROP(21) +ROP(27)&
                   +ROP(31) +ROP(33) +ROP(35) +ROP(36)&
                   -ROP(39) -ROP(40) -ROP(56) -ROP(61)
    source_spec(1,j) = source_spec(1,j) * wmol(1)

    source_spec(2,j) = -ROP(2) +ROP(3) +ROP(5) +ROP(7)&
                   ...
                   +ROP(61) +ROP(70)
    source_spec(2,j) = source_spec(2,j) * wmol(2)

    source_spec(3,j) = -ROP(1) -ROP(1) -ROP(2) -ROP(3)&
                   -ROP(4) -ROP(5) -ROP(6) -ROP(7)&
                   -ROP(8) -ROP(9) -ROP(10) -ROP(11)&
                   -ROP(12) -ROP(13) +ROP(14) +ROP(20)&
                   +ROP(26) +ROP(41)
    source_spec(3,j) = source_spec(3,j) * wmol(3)

    source_spec(4,j) = +ROP(1) +ROP(4) -ROP(14) -ROP(15)&
                   -ROP(16) -ROP(20) +ROP(27) +ROP(42)&
                   +ROP(53) -ROP(55) -ROP(59) -ROP(60)&
                   -ROP(67) -ROP(72) -ROP(73)
    source_spec(4,j) = source_spec(4,j) * wmol(4)

    source_spec(5,j) = +ROP(2) +ROP(3) +ROP(4) +ROP(8)&
                   ...
                   +ROP(59) +ROP(67)
    source_spec(5,j) = source_spec(5,j) * wmol(5)

    source_spec(6,j) = +ROP(25) +ROP(26) +ROP(38) +ROP(40)&
                   +ROP(41) +ROP(42) +ROP(45) +ROP(46)&
                   +ROP(47) +ROP(49) +ROP(50) +ROP(51)&
                   +ROP(60)
    source_spec(6,j) = source_spec(6,j) * wmol(6)

    source_spec(7,j) = -ROP(4) +ROP(15) +ROP(16) -ROP(26)&
                   -ROP(27) -ROP(28) -ROP(42) -ROP(52)&
                   -ROP(53) -ROP(54) +ROP(72) +ROP(73)
    source_spec(7,j) = source_spec(7,j) * wmol(7)

    source_spec(8,j) = -ROP(7) +ROP(8) +ROP(29) -ROP(30)&
                   +ROP(31) +ROP(37) -ROP(45) -ROP(46)&
                   +ROP(47) -ROP(53) +ROP(56) +ROP(57) +ROP(57)&
                   +ROP(61) +ROP(63) +ROP(63) -ROP(67)&
                   -ROP(68) -ROP(69)
    source_spec(8,j) = source_spec(8,j) * wmol(8)

    source_spec(9,j) = -ROP(8) +ROP(30) -ROP(31) -ROP(47)&
                   +ROP(53) -ROP(57) -ROP(63) +ROP(68)&
                   +ROP(69)
    source_spec(9,j) = source_spec(9,j) * wmol(9)
    source_spec(10,j) = +ROP(6) -ROP(9) +ROP(10) -ROP(14)&
                   +ROP(33) -ROP(39) -ROP(48) +ROP(49)&
                   -ROP(54) +ROP(59) +ROP(60) +ROP(66)&
                   +ROP(68) +ROP(70) +ROP(72)
    source_spec(10,j) = source_spec(10,j) * wmol(10)
    source_spec(11,j) = +ROP(9) +ROP(11) +ROP(14) +ROP(48)&
                   +ROP(54) -ROP(66)
    source_spec(11,j) = source_spec(11,j) * wmol(11)
    source_spec(12,j) = +ROP(7) -ROP(12) +ROP(13) -ROP(15)&
                   +ROP(32) -ROP(34) -ROP(35) +ROP(36)&
                   +ROP(39) +ROP(43) +ROP(44) -ROP(50)&
                   +ROP(51) +ROP(52) +ROP(66) +ROP(67)&
                   -ROP(69) +ROP(73)
    source_spec(12,j) = source_spec(12,j) * wmol(12)
    source_spec(13,j) = 0.0_pr



    DO k=1,neqs
      source_spec(k,j) = source_spec(k,j) * FAKWR
    END DO
